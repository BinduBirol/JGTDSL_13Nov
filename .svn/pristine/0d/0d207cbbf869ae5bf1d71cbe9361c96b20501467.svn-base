...
public List<ZoneDTO> getUserZoneList( String whereClause) {
		
		Connection conn = ConnectionManager.getConnection();		
		String sql= " Select * from MST_ZONE  "+whereClause+"  Order by zone_name " ;

		PreparedStatement stmt = null;
		ResultSet r = null;		
		List<ZoneDTO> areaList= new ArrayList<ZoneDTO>();
		try
		{
			stmt = conn.prepareStatement(sql);
			...
			while (r.next())
			{
				ZoneDTO area = new ZoneDTO();
				area.setZone_id(r.getString("zone_id"));
				area.setZone_name(r.getString("zone_name"));
				areaList.add(area);
