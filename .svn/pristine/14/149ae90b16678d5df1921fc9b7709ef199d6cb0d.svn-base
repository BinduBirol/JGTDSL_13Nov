...
public class BankTransactionService {
	...
	public ArrayList<TransactionDTO> getMPGTransactionList(String from_date, String to_date, String bank_name)
	{
		...
		Connection conn = ConnectionManager.getConnection();
		String bank_sql="";
		if(bank_name.equalsIgnoreCase("all")){
			bank_sql= "select bank_id from mst_bank_info where bank_name in (select FULLNAME from MPG_USERINFO )";
		}else{
			bank_sql= "select bank_id from mst_bank_info where bank_name = '"+bank_name+"'";
		}
		 
		
		String sql= " SELECT bal.*,to_char(TRANS_DATE) TRANS_DATE1, getBankBranch(bal.branch_id) bank, " 
					+ " MB.TRANSACTION_ID their_trans_id, getMpgBillMonYear(MB.TRA_ID) as bills "
					+ "  FROM bank_account_ledger bal, mpg_bill mb "
					+ " WHERE     bal.bank_id IN ("+bank_sql+") "
					+ "       AND TRANS_DATE BETWEEN TO_DATE ('"+from_date+"', 'dd-mm-yyyy') "
					+ "                          AND TO_DATE ('"+to_date+"', 'dd-mm-yyyy')"
					+ "  and 'M-'||MB.TRA_ID= BAL.REF_ID order by TRANS_DATE ";
		   ...
		   ResultSet r = null;
			try
			{
				...
				while (r.next())
				{
					...
					transaction.setTrans_date(r.getString("TRANS_DATE1"));
					transaction.setDebit(r.getDouble("DEBIT"));
					transaction.setGas_bill(r.getDouble("ACTUAL_REVENUE"));
					transaction.setSurcharge(r.getDouble("SURCHARGE"));
					transaction.setBank_name(r.getString("BANK"));
					transaction.setTrans_id(r.getString("their_trans_id"));
					transaction.setParticulars(r.getString("bills"));
					transaction.setSl(String.valueOf(sl));					
					transactionList.add(transaction);					
					sl++;
				}
			} 
	...
	public ArrayList<TransactionDTO> getMPGTransactionDateWise(String from_date, String to_date,String bank_name)
	{
		TransactionDTO transaction=null;
		int sl=1;		
		ArrayList<TransactionDTO> transactionList=new ArrayList<TransactionDTO>();
		...
		String sql=  
				 "SELECT TO_CHAR (trans_date) t_trans_date, "
				+ "         SUM (debit) total, "
				+ "         SUM (ACTUAL_REVENUE) t_gas_bill, "
				+ "         SUM (surcharge) t_surcharge, "
				+ "         COUNT (*) customer_count "
				+ "    FROM (SELECT bal.*, "
				+ "                 TO_CHAR (TRANS_DATE) TRANS_DATE1, "
				+ "                 getBankBranch (bal.branch_id) bank, "
				+ "                 MB.TRANSACTION_ID their_trans_id, "
				+ "                 getMpgBillMonYear (MB.TRA_ID) AS bills "
				+ "            FROM bank_account_ledger bal, mpg_bill mb "
				+ "   WHERE     bal.bank_id IN ("+bank_sql+") "
				+ "         AND TRANS_DATE BETWEEN TO_DATE ('"+from_date+"', 'dd-mm-yyyy') "
				+ "                            AND TO_DATE ('"+to_date+"', 'dd-mm-yyyy') "
				+ "                 AND 'M-' || MB.TRA_ID = BAL.REF_ID) "
				+ "GROUP BY trans_date "
				+ "ORDER BY trans_date";
		   ...
		   ResultSet r = null;
			try
			{
				...
				while (r.next())
				{
					...
					transaction.setTrans_date(r.getString("t_trans_date"));
					transaction.setDebit(r.getDouble("total"));
					transaction.setGas_bill(r.getDouble("t_gas_bill"));
					transaction.setSurcharge(r.getDouble("t_surcharge"));
					transaction.setCount(r.getInt("customer_count"));
					transaction.setSl(String.valueOf(sl));					
					transactionList.add(transaction);					
					sl++;
				}
	...
	public ArrayList<TransactionDTO> getMPGTransactionAreaWise(String from_date, String to_date,String bank_name)
	{
		TransactionDTO transaction=null;
		int sl=1;		
		ArrayList<TransactionDTO> transactionList=new ArrayList<TransactionDTO>();
		...
		String sql= "SELECT SUM (bal.debit) a_total, "
				+ "         SUM (bal.ACTUAL_REVENUE) a_gas_bill, "
				+ "         SUM (bal.surcharge) a_surcharge, "
				+ "         SUBSTR (bal.CUSTOMER_ID, 1, 2) area_id, "
				+ "         MA.AREA_NAME, "
				+ "         COUNT (*) customer_count "
				+ "    FROM bank_account_ledger bal, mpg_bill mb, mst_area ma "
				+ "   WHERE     bal.bank_id IN "
				+ "                ("+bank_sql+") "
				+ "         AND TRANS_DATE BETWEEN TO_DATE ('"+from_date+"', 'dd-mm-yyyy') "
				+ "                            AND TO_DATE ('"+to_date+"', 'dd-mm-yyyy') "
				+ "         AND 'M-' || MB.TRA_ID = BAL.REF_ID "
				+ "         AND SUBSTR (bal.CUSTOMER_ID, 1, 2) = MA.AREA_ID "
				+ " GROUP BY SUBSTR (bal.CUSTOMER_ID, 1, 2), MA.AREA_NAME "
				+ " ORDER BY SUBSTR (bal.CUSTOMER_ID, 1, 2)";
		   ...
		   ResultSet r = null;
			try
			{
				...
				while (r.next())
				{
					...
					transaction.setArea_id(r.getString("area_id"));
					transaction.setArea_name(r.getString("area_name"));
					transaction.setDebit(r.getDouble("a_total"));
					transaction.setGas_bill(r.getDouble("a_gas_bill"));
					transaction.setSurcharge(r.getDouble("a_surcharge"));
					transaction.setCount(r.getInt("customer_count"));
					transaction.setSl(String.valueOf(sl));					
					transactionList.add(transaction);					
					sl++;
	...
	public ArrayList<TransactionDTO> getUnAuthCount(String area_id)
	{
		...
		"Where  bank.area_id=? AND tmp1.account_no= account.account_no  " +
