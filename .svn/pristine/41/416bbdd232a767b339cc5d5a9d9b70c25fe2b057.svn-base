...
public class MinistryCollectionReport extends BaseAction {
	...
	public ArrayList<MinistryCollectionDTO> getMinistryCollection() {
		...
		String sql = "SELECT BAL.CUSTOMER_ID, "
				+ "         CC.MINISTRY_ID, "
				+ "         MM.MINISTRY_NAME, "
				+ "         TO_CHAR (BAL.TRANS_DATE) tran_date, "
				+ "         getbankbranch (bal.BRANCH_ID) bank, "
				+ "         BAL.Debit, MCC.CATEGORY_NAME "
				+ "    FROM BANK_ACCOUNT_LEDGER bal, CUSTOMER_CONNECTION cc, MST_MINISTRY mm , MST_CUSTOMER_CATEGORY mcc"
				+ "   WHERE     TO_CHAR (TRANS_DATE, 'MM') =  " + coll_month
				+ "         AND TO_CHAR (TRANS_DATE, 'YYYY') =  " + coll_year
				+ "         AND BAL.CUSTOMER_ID = CC.CUSTOMER_ID and MCC.CATEGORY_ID= substr(CC.CUSTOMER_ID,3,2)"
				+ "         AND CC.MINISTRY_ID = MM.MINISTRY_ID "
				+ "         AND CC.MINISTRY_ID IS NOT NULL "
				+ "         AND SUBSTR (BAL.CUSTOMER_ID, 1, 2) =  "
				+ loggedInUser.getArea_id() + "ORDER BY tran_date asc ";
	...
	public ArrayList<MinistryCollectionDTO> getMinistryCollectionTotal() {
		...
		String sql =  "SELECT CC.MINISTRY_ID, MM.MINISTRY_NAME, SUM (BAL.Debit) t_debit, count(*) count "
				+ "    FROM BANK_ACCOUNT_LEDGER bal, CUSTOMER_CONNECTION cc, MST_MINISTRY mm "
				+ "   WHERE     TO_CHAR (TRANS_DATE, 'MM') = " + coll_month
				+ "         AND TO_CHAR (TRANS_DATE, 'YYYY') = " + coll_year
				+ "         AND BAL.CUSTOMER_ID = CC.CUSTOMER_ID "
				+ "         AND CC.MINISTRY_ID = MM.MINISTRY_ID "
				+ "         AND CC.MINISTRY_ID IS NOT NULL "
				+ "         AND SUBSTR (BAL.CUSTOMER_ID, 1, 2) = "+ loggedInUser.getArea_id()
