...
public class ReconnectionService {
	...
	public ResponseDTO saveMeterReconnectInfo(ReconnectDTO reconn,DisconnectDTO disconn)
	{
		...
		Statement reading_id_stmt= null;
		ResultSet reading_id_rs= null;
		...
		String sqn_reading_id_q= " select SQN_METER_READING.nextval reading_id from dual ";
		int reading_id= 0;
		...
		*/
		...
		String sqlUpdate=" Update CUSTOMER_METER set status=? where customer_id=? and meter_id=?";
			...
			try
			{
				reading_id_stmt= conn.createStatement();
				reading_id_rs= reading_id_stmt.executeQuery(sqn_reading_id_q);
				while(reading_id_rs.next()){
					reading_id= reading_id_rs.getInt("reading_id");
				}				
				...
				stmt.setInt(4,ReadingPurpose.RECONNECT_METER.getId());
				stmt.setString(5,reconn.getMeter_reading());
				stmt.setString(6,reconn.getReconnect_date());
				stmt.setString(7,"Reconnect Meter");
				stmt.execute();
