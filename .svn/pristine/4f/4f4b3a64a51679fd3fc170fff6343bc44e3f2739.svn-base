...
//current system doesn't have data for paid_due_amount
			...
			if (reconnectionDto.getReconn_cause()== null){
				pcell = new PdfPCell(new Paragraph(DisconnCause.values()[ Integer.valueOf(0)].getLabel(),ReportUtil.f8));
			}else{
	...
	ArrayList<NonMeterReportDTO> reconnectionList=new ArrayList<NonMeterReportDTO>();
		...
		try {
			...
			String defaulterSql="SELECT bqc.CUSTOMER_ID, " +
					...
					//" and NEW_RECONN_QNT_4M_PERMANENT>0   " +
					"" +wClause +" and BQC.RECONNECTION_CAUSE is not null order by EFFECTIVE_DATE";
//					"union all " +
...
//					"order by CATEGORY_ID,EFFECTIVE_DATE,CUSTOMER_ID ";
	...
	ArrayList<NonMeterReportDTO> disconnCustomerList=new ArrayList<NonMeterReportDTO>();
		...
		try {
			...
			String defaulterSql=
					" SELECT CAST(to_char(LAST_DAY(to_date('01-'||to_char(to_date(RECONNECT_DATE,'dd-MM-YYYY'), 'mm')||'-'||to_char(to_date(RECONNECT_DATE,'dd-MM-YYYY'), 'YYYY'),'dd-MM-YYYY')),'dd') AS INT) LASTDAY, " +
					"         rm.CUSTOMER_ID, " +
					"         mci.FULL_NAME, " +
					"         mci.CATEGORY_ID, " +
					"         mci.CATEGORY_NAME, " +
