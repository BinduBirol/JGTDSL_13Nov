...
//			return response;
	...
	public ArrayList<ClearnessDTO> getDueMonthWeb(String customer_id){
		...
		try {
			String transaction_sql="SELECT customer_id, BILL_ID, BILL_MONTH, BILL_YEAR, TO_CHAR (TO_DATE (BILL_MONTH, 'MM'), 'MON')||''''|| SUBSTR (BILL_YEAR, 3) MONTH_YEAR,PAYABLE_AMOUNT, " +
					"0 PAID_AMOUNT, PAYABLE_AMOUNT DUE_AMOUNT,  " +
					"						CALCUALTESURCHARGE_METER(BILL_ID,to_char(sysdate,'dd-mm-yyyy')) SURCHARGE"+
					"						FROM BILL_METERED  " +
					"						WHERE status = 1 AND customer_id ='"+customer_id+"'  " +
					"						UNION  " +
					"						SELECT customer_id,  " +
	...
	public ArrayList<ClearnessDTO> getDueMonthWeb(String customer_id, String billIds){
		...
		try {
			String transaction_sql="select customer_id, BILL_ID, BILL_MONTH, BILL_YEAR,MONTH_YEAR,PAYABLE_AMOUNT,PAID_AMOUNT,DUE_AMOUNT,CALCUALTESURCHARGE_METER(BILL_ID,to_char(sysdate,'dd-mm-yyyy')) SURCHARGE from( " +
					...
					"COLLECTED_AMOUNT PAID_AMOUNT, PAYABLE_AMOUNT- NVL(COLLECTED_AMOUNT,0) DUE_AMOUNT  " +
					"						FROM BILL_METERED   " +
