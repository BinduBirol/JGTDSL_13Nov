...
<div id="sw-basic-step-4">
    ...
    <div class="row-fluid">
                ...
                <div class="w-box-content cnt_a">
                    ...
                    <div class="row-fluid">
                        <div class="span12">
                            ...
                            <s:if test="customer.connectionInfo.connection_type == null">
	                            <select name="customer.connectionInfo.connection_type_str" id="connection_type" style="width: 39%" >
                   ...
                   <div class="row-fluid" id="sub_connection_div">
	                        <div class="span12">
	                            <label>Sub Connection<m class='man'/><font style="font-size: 8px">(cond.)</font></label>
	                            <s:if test="%{customer.connectionInfo.hassub_connection == 'N' || !customer.connectionInfo.hassub_connection == null}"> 
	                            	 <input type="text" id="parent_connection" style="font-weight: bold;color: #3b5894;position: absolute; z-index: 2; background: transparent;width: 17%;margin-top: -4px;" name="customer.connectionInfo.parent_connection"/>
									<input type="text" name="" id="parent_connection_x" disabled="disabled" style="color: #CCC; position: absolute; background: transparent; z-index: 1;border: none;width: 17%;margin-top: -5px;"/> 	                            
	                             </s:if> 
	                            <s:else>
	                            	<s:property value="customer.connectionInfo.sub_connection"/>
	                            </s:else>	                            
	                        </div>
	                </div>
      ...
      <div style="padding-top: 10px;" class="span6" id="deposit_div">
            <div class="w-box">
                <div class="w-box-header">
                    <h4>Deposit Info.</h4>                    
                </div>                
                <div class="w-box-content cnt_a">                                       
                    <div class="row-fluid">
                        <div class="span12" id="deposit_info_div">
                    	</div>                    	
                    </div>                                        
                </div>
            </div>
	...
	<div class="span2">
		<textarea name="customer_note" id="editableNote" placeholder="add a note for this customer"><s:property value="customer_note" /></textarea>
		
		<button id="updateNote" class="btn-info" type="button">Add/Update</button>
	</div>


	<script type="text/javascript">
        	ajaxLoad("deposit_info_div","getDepositInfo.action?customer_id=<s:property value='customer.customer_id' />");
</script>
...
*/
<s:if test="%{customer.connectionInfo.parent_connection == null}">
	$("#parent_connection_div").hide();                
</s:if>
...
$("#updateNote").click(function(){
	...
	var note=$("#editableNote").val();
	var customer_id= $("#customer_id").val();


	$.ajax({
	    url: 'updateCustomerNote.action?customer_id='+customer_id+'&customer_note='+note,
	    type: 'POST',
