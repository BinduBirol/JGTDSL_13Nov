...
//end of : testing for codeless collection
	...
	public ResponseDTO saveBillCollection(CollectionDTO collection,boolean mobilePhoneUpdate)
	{
		...
		String sql_update_metered="Update BILL_METERED Set Status=2,Collected_amount=nvl(collected_amount,0)+?,COLLECTION_DATE=to_date(?,'dd-MM_YYYY'),BRANCH_ID=?, COLLECTED_SURCHARGE= nvl(COLLECTED_SURCHARGE, 0)+ ?  Where Bill_Id=?";
		...
		String bank_book_info_select="Select Payable_Amount,GAS_BILL,BILL_METERED.METER_RENT,BILL_METERED.SURCHARGE_AMOUNT,BILL_METERED.DEMAND_CHARGE  " +
		...
		String bal_trans_id="";
		double payableAmount=new Double(0),gasBill=new Double(0),meterRent=new Double(0),surchargeAmount=new Double(0),demandCharge=new Double(0);
		...
		try
			{
				...
				if (r.next()){
					...
					demandCharge=r.getDouble("DEMAND_CHARGE");
				}
				...
				if(isMeteredStatus.equalsIgnoreCase("1")){
					stmt = conn.prepareStatement(sql_update_metered);
					 if(collection.getCollected_amount()<collection.getPayable_amount()){
						...
						double tax_am=collection.getTax_amount();
						double miss_amount=collection.getPayable_amount()-collection.getCollected_amount();
						double amount_with_tax= collection.getCollected_amount()+ tax_am;
						double col_miss_s_amount_w_tax=collection.getPayable_amount()-amount_with_tax;
						double total_sur_coll=surchargeAmount-col_miss_s_amount_w_tax;
						//total demand+surcharge+tax amount
						double to_de_su_ta_amount=demandCharge+tax_am+surchargeAmount;
						//total surcharge+tax amount
						double to_sur_tax_amount=tax_am+surchargeAmount;
						
						double to_demand_tax=demandCharge+tax_am;
					//// without demand_charge+ partial surcharge
						double amount=collection.getPayable_amount()-amount_with_tax;
						double partial_sur_coll=amount-demandCharge;
						double to_parSuColl_demand=partial_sur_coll+tax_am+demandCharge;
						
						if((miss_amount==tax_am)||(miss_amount==to_demand_tax)||(miss_amount!=to_sur_tax_amount)){
							stmt.setDouble(4,surchargeAmount);
						}else if((miss_amount==to_de_su_ta_amount)||(miss_amount==to_sur_tax_amount)){
							stmt.setDouble(4,0.0);
							
						}else if(miss_amount==to_parSuColl_demand){
							stmt.setDouble(4,partial_sur_coll);
							
						}
						
						
						else{
							stmt.setDouble(4,total_sur_coll);
						}
						stmt.setDouble(1,collection.getCollected_amount()+tax_am);
						stmt.setString(2, collection.getCollection_date());
						stmt.setString(3,collection.getBranch_id());
						stmt.setString(5,collection.getBill_id());
			...
			/*	bank_book_stmt = conn.prepareStatement(bank_book_info_select);				
				...
				if(isMeteredStatus.equalsIgnoreCase("1")){
					...
					double excludingSurcharge=0.0;
					
					//System.out.println(collection.getCollected_amount()+"<c\tp>"+collection.getPayable_amount());
					
					if(collection.getCollected_amount()>collection.getPayable_amount()&&collection.getTax_amount()>0.0)
					{
					...
					}else if(collection.getCollected_amount()<collection.getPayable_amount()){	
						
						double coll_missmatch_amount= collection.getPayable_amount()-collection.getCollected_amount();
						double tax_am=collection.getTax_amount();
						...
						if((coll_missmatch_amount==tax_am)||(coll_missmatch_amount==to_demand_tax)||(coll_missmatch_amount!=to_de_su_ta_amount)){
							...
							bank_book_stmt_update_metered.setDouble(3,0.0);
						}
						...
						/*	double miss_amount=collection.getPayable_amount()-collection.getCollected_amount();
							...
							bank_book_stmt_update_metered.setDouble(3,total_sur_coll);
							}
	...
	/////////////////////ministry collection begins
	
	public ResponseDTO saveBillCollectionMinistry(CollectionDTO collection)
	{
		ResponseDTO response=new ResponseDTO();
		TransactionManager transactionManager=new TransactionManager();
		Connection conn = transactionManager.getConnection();
		...
		int total_amount=0;
		String customer_id_in_condition=null;
		String [] ta = null;		
		for(String x:collection.getBilled_amounts()){
			ta= x.split(",");
		}
		for(String x:collection.getCustomer_ids()){
			customer_id_in_condition=x;
		}
		for(String x:ta){			
			total_amount = total_amount+Integer.parseInt(x);
		}	
				
		String non_meter_update_query=  " UPDATE bill_non_metered " +
										"   SET COLLECTION_DATE = TO_DATE (?, 'DD-MM-YYYY'), " +
										" COLLECTED_SURCHARGE= nvl(ACTUAL_SURCHARGE,0), "+
										"       STATUS = 2, " +
										"       COLLECTED_BILLED_AMOUNT = NVL (BILLED_AMOUNT, 0), " +
										"       COLLECTED_PAYABLE_AMOUNT = NVL (ACTUAL_PAYABLE_AMOUNT, 0) " +
										" WHERE     bill_year || LPAD (bill_month, 2, 0) BETWEEN ? AND ? " +
										"       AND customer_id IN ("+customer_id_in_condition+") AND status = 1 " ;
		
		String meter_update_query= 		" UPDATE bill_metered " +
										"   SET COLLECTED_AMOUNT = NVL (BILLED_AMOUNT, 0), " +
										"       COLLECTION_DATE = TO_DATE (?, 'DD-MM-YYYY'), " +
										"       STATUS = 2, " +
										"       COLLECTED_SURCHARGE = NVL (SURCHARGE_AMOUNT, 0) " +
										" WHERE     bill_year || LPAD (bill_month, 2, 0) BETWEEN ? AND ? " +
										"       AND customer_id IN ("+customer_id_in_condition+") " +
										"       AND status = 1 " ;
		
		String insert_ministry_query= 	" INSERT INTO BILL_COLLECTION_MINISTRY (JV_NO,CUSTOMER_IDS, FROM_MONTH,FROM_YEAR,TO_MONTH,TO_YEAR,MINISTRY_ID,TOTAL_COLLECTED_AMOUNT,REMARKS,COLLECED_BY,INSERTED_ON,COLLECTION_DATE) " +
										" VALUES (?,?,?,?,?,?,?,?,?,?, SYSDATE ,TO_DATE ( '"+collection.getCollection_date()+"', 'DD-MM-YYYY')) " ;

		
		PreparedStatement non_meter_update_stmt = null;
		PreparedStatement meter_update_stmt = null;
		PreparedStatement insert_ministry_stmt = null;
		
		String from_year_month= "";
		
		if(collection.getMonth_from().length()==1){
			from_year_month= collection.getYear_from()+"0"+collection.getMonth_from();
		} else{
			from_year_month= collection.getYear_from()+collection.getMonth_from();
		}
		
		String to_year_month= "";
		if(collection.getMonth_to().length()==1){
			to_year_month= collection.getYear_to()+"0"+collection.getMonth_to();
		} else{
			to_year_month= collection.getYear_to()+collection.getMonth_to();
		}

		try {
			
			insert_ministry_stmt= conn.prepareStatement(insert_ministry_query);			
			insert_ministry_stmt.setString(1, collection.getJbNo());
			String customer_ids_for_table= customer_id_in_condition.replace("'", "");
			insert_ministry_stmt.setString(2, customer_ids_for_table);
			insert_ministry_stmt.setString(3, collection.getMonth_from());
			insert_ministry_stmt.setString(4, collection.getYear_from());
			insert_ministry_stmt.setString(5, collection.getMonth_to());
			insert_ministry_stmt.setString(6, collection.getYear_to());
			insert_ministry_stmt.setString(7, collection.getMINISTRY_ID());			
			insert_ministry_stmt.setInt(8,total_amount);
			insert_ministry_stmt.setString(9, collection.getRemarks());
			insert_ministry_stmt.setString(10, collection.getInserted_by());
	
			insert_ministry_stmt.executeUpdate();			
			
			non_meter_update_stmt= conn.prepareStatement(non_meter_update_query);			
			non_meter_update_stmt.setString(1, collection.getCollection_date());
			non_meter_update_stmt.setString(2, from_year_month);
			non_meter_update_stmt.setString(3, to_year_month);	
			non_meter_update_stmt.executeUpdate();	
			
			meter_update_stmt= conn.prepareStatement(meter_update_query);
			meter_update_stmt.setString(1, collection.getCollection_date());	
			meter_update_stmt.setString(2, from_year_month);
			meter_update_stmt.setString(3, to_year_month);			
			meter_update_stmt.executeUpdate();
			
			
			
			transactionManager.commit();
			
			response.setMessasge("Successfully saved Ministry collection information.<br/>Total amount:<b style='color:green;'> "+total_amount+"</b> Tk.");
			response.setResponse(true);
 		...
 		finally{try{non_meter_update_stmt.close();meter_update_stmt.close();insert_ministry_stmt.close();transactionManager.close();} catch (Exception e)
			{e.printStackTrace();}insert_ministry_stmt = null;non_meter_update_stmt=null;meter_update_stmt=null;conn = null;}
 		
		return response;

	}
	
	////////////////////ministry collection ends
...
////////////////// BY Toraf////////////////////////
	...
	public ArrayList<CollectionDTO> getCustomerCollection4Delete(String customer_id) {
		CollectionDTO collection=new CollectionDTO();
		ArrayList<CollectionDTO> collectionList=new ArrayList<CollectionDTO>();

		Connection conn = ConnectionManager.getConnection();
		String sql="SELECT scroll_no, to_char(COLLECTION_DATE) coll_date, " +
					"       customer_id, " +
					"       customer_name, " +
					"       month_from, " +
					"       year_from, " +
					...
					"       collected_surcharge, " +
					"       bank_id, " +
					"       branch_id, getBankbranch(branch_id) bank_name " +
					"  FROM daily_collection_grid " +
		...
		ResultSet r = null;
			...
			try
			{
				...
				{	
					...
					collection.setScroll_no(r.getString("SCROLL_NO"));
					collection.setCollection_date(r.getString("coll_date"));
					collection.setCustomer_id(r.getString("CUSTOMER_ID"));
					collection.setCustomer_name(r.getString("CUSTOMER_NAME"));
					collection.setMonth_from(r.getString("MONTH_FROM"));
					collection.setMonth_to(r.getString("MONTH_TO"));
