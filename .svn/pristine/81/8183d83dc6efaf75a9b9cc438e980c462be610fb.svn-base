...
//					ptable.addCell(pcell);
			...
			ptable = new PdfPTable(8);
			ptable.setWidthPercentage(100);
			ptable.setWidths(new float[]{15,24,45,55,30,30,30,40});
			ptable.setSpacingBefore(10);
...
//			ptable.addCell(pcell);
			...
			pcell=new PdfPCell(new Paragraph("Due months",ReportUtil.f9B));
			pcell.setMinimumHeight(18f);
			pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
			pcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
			ptable.addCell(pcell);
...
//			
			...
			if(loadIncraseDTO.getDue_amount().length()>3){					
				String da []=loadIncraseDTO.getDue_amount().split("#");	
				if(da[0].isEmpty())da[0]="0";
				dueAmount=Double.parseDouble(da[0]);	
			...
			pcell = new PdfPCell(new Paragraph(due_month,ReportUtil.f8));
			pcell.setMinimumHeight(16f);
			pcell.setHorizontalAlignment(Element.ALIGN_LEFT);
			pcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
			ptable.addCell(pcell);
...
//,BQC.APPLIANCE_TYPE_CODE IS ADDED TO PARTIAL BILL CALC.			
			String defaulterSql="SELECT bqc.CUSTOMER_ID, " +
					...
					"          Partial_bill ,REMARKS,BQC.DISCONN_TYPE,DISCONN_CAUSE ,getDueMonth (bqc.CUSTOMER_ID,TO_CHAR (bqc.EFFECTIVE_DATE, 'YYYY')|| TO_CHAR (bqc.EFFECTIVE_DATE, 'mm')) due_month_tk " +
					...
					"       AND SUBSTR (bqc.customer_id, 3, 2) = MCC.CATEGORY_ID " +
					w2Clause+
					"       and NEW_TEMPORARY_DISCONN_QNT>0   " +wClause+ " and DISCONN_CAUSE is not null "+
...
//					" order by CATEGORY_ID,DISCONN_TYPE,DISCONN_CAUSE,EFFECTIVE_DATE_ORDER ,CUSTOMER_ID" ;
        	...
        	while(resultSet.next())
        	{
        		...
        		loadIncraseDTO.setDue_amount(resultSet.getString("due_month_tk"));
        		loadIncraseDTO.setDue_months(resultSet.getString("due_month_tk"));
