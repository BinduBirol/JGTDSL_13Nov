...
public class MaximumLoadExceedReport extends BaseAction {
	...
	public String execute() throws Exception
	{
		...
		try{
			...
			if(report_for.equals("area_wise")&& report_for2.equals("month_wise") ) {
			
			pcell= new PdfPCell(new Paragraph("Month-Year: "+Month.values()[Integer.valueOf(bill_month)-1]+", "+bill_year,ReportUtil.f8B));
			pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
			pcell.setColspan(3);
			pcell.setPadding(5);
			...
			if(report_for.equals("area_wise")&& report_for2.equals("year_wise") ) {
				
				pcell= new PdfPCell(new Paragraph("Year "+bill_year,ReportUtil.f8B));
				pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
				pcell.setColspan(3);
				pcell.setPadding(5);
				pcell.setBorder(Rectangle.NO_BORDER);
