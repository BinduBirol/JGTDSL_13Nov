...
public class BillMeteredCustomer extends BaseAction implements
	...
	public String downloadBill() throws Exception {
		...
		try {
			...
			for (MBillDTO bill : billList) {
				...
				// bindu
				String vat_no ="VAT NO.";				
				over.setTextMatrix(367, 503);
				/*
				if(loggedInUser.getArea_id().equals("10")){						
					vat_no= "701157681";
				}
				if(loggedInUser.getArea_id().equals("06")){						
					vat_no= "7011057681";
				}
				*/
				over.showText("701157681");
				...
				over.setFontAndSize(bf, 9);
				over.showTextAligned(PdfContentByte.ALIGN_LEFT,
						consumption_format.format(String.valueOf(bill.getMax_load())), 127,
						226, 0);// 227
				...
				over.setFontAndSize(bf, 9);
				over.showTextAligned(PdfContentByte.ALIGN_LEFT, "/Max Load: ", 60,226, 0);// 227
				
				over.setFontAndSize(bf, 9);
				over.showTextAligned(PdfContentByte.ALIGN_LEFT, 
						consumption_format.format(bill.getMax_load()), 127,
						226, 0);// 227
				
				over.setFontAndSize(bfBold, 10);
				over.showTextAligned(PdfContentByte.ALIGN_LEFT,
						"Taka "+bill.getAmount_in_word(), 114, 177, 0);
				...
				over.setFontAndSize(bf, 9);
				over.showTextAligned(PdfContentByte.ALIGN_RIGHT, "/Demand Charge:", 380,
						343, 0);
				...
				*/
				...
				over.setFontAndSize(bf, 9);
				over.showTextAligned(PdfContentByte.ALIGN_RIGHT, taka_format
						.format(bill.getDemand_charge()), 442,
						343, 0);
				...
				// others
				/*
				if(bill.getPbMarginDTO().getHhv_nhv_bill()>0){
					over.showTextAligned(PdfContentByte.ALIGN_RIGHT,
							taka_format.format(bill.getPbMarginDTO().getHhv_nhv_bill()),
							442, 221, 0);// 220
				}else{*/
					over.showTextAligned(PdfContentByte.ALIGN_RIGHT,
				...
				double totalBill = gas_bill
						+ bill.getPbMarginDTO().getMin_load_bill()
						+ bill.getGovtMarginDTO().getTotal_amount()
						+ bill.getPbMarginDTO().getMeter_rent()+bill.getDemand_charge();
				over.setFontAndSize(bf, 9);
				// total bill
				over.showTextAligned(PdfContentByte.ALIGN_RIGHT,
		...
		try {
			...
			for (MBillDTO bill : billList) {
				...
				if (!cur_category.equals(prev_category)) {
					...
					pcell = new PdfPCell(new Paragraph("Max Load", font9B));
					pcell.setRowspan(1);
					pcell.setPadding(5);
					pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
					datatable.addCell(pcell);
					...
					pcell = new PdfPCell(new Paragraph("D.C.", font9B));
					pcell.setRowspan(1);
					pcell.setPadding(5);
					pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
					datatable.addCell(pcell);
				...
				for (int i = 0; i < readingList.size(); i++) {
					...
					pcell = new PdfPCell(new Paragraph(taka_format.format(bill.getDemand_charge()), font9));
					pcell.setRowspan(1);
					pcell.setPadding(5);
					pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
					datatable.addCell(pcell);
	...
	public ArrayList<MBillDTO> getBillprev(String customer_category,
		...
		String sql = "SELECT brm.READING_ID,  "
				...
				+ "       mr.PRESSURE_FACTOR,  "
				+ "       mr.HHV_NHV,  "
				+ "       mr.RATE, MR.PMAX_LOAD, "
				+ "       tmp1.*  "
				+ "  FROM BILLING_READING_MAP brm,  "
				+ "       METER_READING mr,  "
				+ "       (SELECT nvl(demand_charge,0) demand_charge , bill.bill_id,  "
				+ "               bill_month,  "
				+ "               bill_year,  "
				+ "               bill.customer_id,  "
	...
	public ArrayList<PreviewTotalDTO> getBillprevTotal(String customer_category,
		...
		String sql = 				
						...
						"         CUSTOMER_CATEGORY, " +
						"         SUM (PAYABLE_AMOUNT+nvl((demand_charge),0)) total_to_pay, " +
						"         SUM (BILLED_CONSUMPTION) AS total_consumption, " +
						"         COUNT (CUSTOMER_ID) AS customer_cnt,  nvl(sum(demand_charge),0) demand_charge " +
						"    FROM bill_metered " +
		...
		try {
			...
			while (r.next()) {
				...
				previewTotalDTO.setTotal_dc(r.getDouble("demand_charge"));
