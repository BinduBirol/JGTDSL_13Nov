...
public class DefaulterCustomerList extends BaseAction {
	...
	public String execute() throws Exception {
		...
		try {
			...
			if(report_for.equals("all_area")){
				chunk1 = new Chunk("Head", ReportUtil.f10B);
				chunk2 = new Chunk("Office", ReportUtil.f10B);
			}else{
				chunk1 = new Chunk("Regional Office :", ReportUtil.f10B);
				chunk2 = new Chunk(String.valueOf(Area.values()[Integer
			...
			double subTotal_sec_amount = 0.0;
			int subTotal_month = 0;
			float grandTotal_amount = 0;
			double grandTotal_sec_amount =0.0;
			...
			if (expireListSize==0){
				...
				ptable.setWidths(new float[] { 8, 20, 50, 15, 15,55, 20, 20,20 });
...
//				}
				...
				pcell = new PdfPCell(new Paragraph("Sec Deposit",
						ReportUtil.f10B));
				pcell.setMinimumHeight(18f);
				pcell.setRowspan(2);
				pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
					...
					if (!currentMoholla.equals(previousMoholla)) {
						...
						if (!(previousMoholla.equals("") && currentMoholla
							...
							if (i > 0) {
								...
								pcell = new PdfPCell(new Paragraph(taka_format.format(subTotal_amount),
										ReportUtil.f10B));
								pcell.setMinimumHeight(18f);
								pcell.setColspan(1);
								pcell.setBorder(0);
								...
								pcell = new PdfPCell(new Paragraph(
										taka_format.format(subTotal_sec_amount),
										ReportUtil.f10B));
								pcell.setMinimumHeight(18f);
								pcell.setColspan(1);
								pcell.setBorder(3);
...
//						}
				...
				String name_address = "" + defaulterDto.getFull_name() + ",\n"
						+ defaulterDto.getAddress()+",\nMobile:"+defaulterDto.getContact_no();
				...
				subTotal_sec_amount = subTotal_sec_amount + defaulterDto.getSec_deposit();
				grandTotal_sec_amount=grandTotal_sec_amount+defaulterDto.getSec_deposit();
				...
				pcell = new PdfPCell(new Paragraph(
						taka_format.format(defaulterDto.getSec_deposit()),
						ReportUtil.f10B));
				pcell.setMinimumHeight(16f);
				pcell.setHorizontalAlignment(Element.ALIGN_LEFT);
				pcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
			...
			pcell = new PdfPCell(new Paragraph(taka_format.format(subTotal_amount), ReportUtil.f10B));
			pcell.setMinimumHeight(18f);
			pcell.setColspan(3+space);
			pcell.setBorder(2);
			pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
			pcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
			ptable.addCell(pcell);
			...
			pcell = new PdfPCell(new Paragraph(
					taka_format.format(subTotal_sec_amount), ReportUtil.f10B));
			pcell.setMinimumHeight(18f);
			pcell.setColspan(1);
			pcell.setBorder(2);
			pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
			...
			pcell = new PdfPCell(new Paragraph(taka_format.format(grandTotal_amount), ReportUtil.f10B));
			pcell.setMinimumHeight(18f);
			...
			pcell = new PdfPCell(new Paragraph(
					taka_format.format(grandTotal_sec_amount), ReportUtil.f10B));
			pcell.setMinimumHeight(18f);
			pcell.setColspan(1);
			pcell.setBorder(2);
			pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
	...
	private ArrayList<DefaulterDto> getDefaulterInformationP() {
		ArrayList<DefaulterDto> defaulterInfoList = new ArrayList<DefaulterDto>();

		try {
			String monyear="";
			if(bill_month.length()<2){
				monyear = String.valueOf(bill_year) + "" + "0"+ String.valueOf(bill_month);
			}else{
				monyear = String.valueOf(bill_year)+ String.valueOf(bill_month);
			}
			...
			} else if (report_for.equals("all_area")){
				wClause="";
			}
			...
			if(customer_type.equals("metered") && report_for.equals("all_area")){
				
				defaulterListSql = "SELECT tmp1.CUSTOMER_ID, round(nvl(getDeposit (tmp1.CUSTOMER_ID),0)) total_deposit, "
				...
				defaulterListSql += "      tmp2.CONTACT_NO,"
						...
						+ "                 bi.AREA_ID, "
						+ "                 cc.STATUS, "
						+ "  getDueMonthforBalancing(bi.CUSTOMER_ID,"+month_year+") AS DUEMONTH,    "               
		                + "  getBalance(bi.CUSTOMER_ID,"+month_year+") TOTAL_AMOUNT, "
		                + "  getDueMonthCountforBalancing(bi.CUSTOMER_ID,"+month_year+") TOTAL_MONTH "
				...
				if (moholla_wise.equals("1")) {
					...
					defaulterListSql += " ORDER BY tmp1.CATEGORY_ID , tmp1.CUSTOMER_ID ASC, tmp1.STATUS desc ";
				}
				
			} else if(customer_type.equals("nonmetered") && report_for.equals("all_area")){
			...
			} else if (customer_type.equals("metered")) {
				// query has been changed according to current needs ~ dec 14 ~
				// Prince

				defaulterListSql = "SELECT tmp1.CUSTOMER_ID, getDeposit(tmp1.CUSTOMER_ID) total_deposit, "
						+ "				 tmp1.CATEGORY_ID," + "				 tmp2.CATEGORY_NAME,"
						+ "				 tmp1.STATUS," + "				 TMP2.FULL_NAME,"
						+ "				 tmp2.ADDRESS,";
				if (moholla_wise.equals("1")) {
			...
			while (resultSet.next()) {
				...
				defaulterDto.setSec_deposit(resultSet.getDouble("total_deposit"));
				defaulterDto.setContact_no(resultSet.getString("CONTACT_NO"));
	...
	private ArrayList<DefaulterDto> getDefaulterInformation() {
		...
		try {
			...
			if (report_for.equals("area_wise")) {
				wClause = " and SUBSTR (CUSTOMER_ID, 1, 2) = '" + area + "'";
			} else if (report_for.equals("category_wise")) {
				wClause = " and SUBSTR (CUSTOMER_ID, 1, 2) = '" + area + "' And SUBSTR (CUSTOMER_ID, 3, 2)= '"
						+ customer_category +"'";
			...
			if (criteria_type.equals("lt")) {
				criteriaWclause = " AND getDueMonthCountforBalancing (CUSTOMER_ID, "+month_year+") <" + month_number;
			} else if (criteria_type.equals("gt")) {
				criteriaWclause = " AND getDueMonthCountforBalancing (CUSTOMER_ID, "+month_year+") >" + month_number;
			} else if (criteria_type.equals("eq")) {
				criteriaWclause = " AND getDueMonthCountforBalancing (CUSTOMER_ID, "+month_year+") =" + month_number;
			...
			if(customer_type.equals("metered") && report_for.equals("all_area")){
				...
				defaulterListSql += "      tmp2.CONTACT_NO,"
					    ...
					    + "  SUBSTR (CUSTOMER_ID, 1, 2) AREA_ID, "
					    + "  cc.STATUS, "
						+ "  getDueMonthforBalancing(CUSTOMER_ID,"+month_year+") AS DUEMONTH,    "               
		                + "  getBalance(CUSTOMER_ID,"+month_year+") TOTAL_AMOUNT, "
		                + "  getDueMonthCountforBalancing(CUSTOMER_ID,"+month_year+") TOTAL_MONTH "
						
		                
		                
		                + " FROM CUSTOMER_CONNECTION cc "
						+ " WHERE     cc.ISDELETED <> 'Y' "
						+ wClause
						+ criteriaWclause
						+ status_Sql
						+ "      AND ismetered = 1 "  
