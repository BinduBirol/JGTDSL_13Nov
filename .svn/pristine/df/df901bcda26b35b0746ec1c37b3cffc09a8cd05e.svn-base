...
var addParams={
        ...
        beforeShowForm: function( formId ) {
		   ...
		   $.ajax({type: 'POST',url: jsEnum.NEXT_ID_ACTION,
		   		  ...
		   		  success:function(data){if(data.status=="OK")$('#account_no', formId).val(data.message);else{$("#account_no").focus();alert(jsMsg.ERROR_NEXT_ID);}},
		   		  error:function(){}
			   });
...
jQuery("#gridTable")
    .jqGrid({
        ...
        colModel: [{
		                ...
		                width:100,
		                search: true,
		                editoptions: { readonly: jsEnum.READ_ONLY,maxlength: 25 },
