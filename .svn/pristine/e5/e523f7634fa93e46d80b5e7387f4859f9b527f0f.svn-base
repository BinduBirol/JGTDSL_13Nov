				...
				<div class="w-box-content" style="padding: 10px;" id="content_div">												
						...
						<div class="row-fluid">
							<div class="span12">
								...
								<input type="radio" name="operationType" id="adjustment" value="adjustment" checked="checked" onclick="changeSelection(this.value)"/>&nbsp;&nbsp;Adjustment
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;								
								<input type="radio" name="operationType" id="othersAmount" value="othersAmount" onclick="changeSelection(this.value)" />
								&nbsp;&nbsp;Others Amount
						  	</div>
						</div>
			...
			<div class="w-box">
				...
				<div class="w-box-content" style="padding: 10px;" id="content_div">						
						<div class="row-fluid">
						 ...
						 <div id="adjustment_div">
							 ...
							 <div class="row-fluid">
								<div class="span12">
									...
									<input type="button" name="+" value="+" onclick="$('#adjustment_sign').val('+');setNewPayableAmount();" style="width: 32px;height: 26px;margin-bottom: 0px;font-weight: bold;border:2px solid red;" /> / 
									<input type="button" name="-" value="-" onclick="$('#adjustment_sign').val('-');setNewPayableAmount();" style="width: 32px;height: 26px;margin-bottom: 0px;font-weight: bold;border:2px solid green;" />
									<input type="text" style="width: 4%;text-align: center;font-weight: bold;"  id="adjustment_sign" readonly/>
									<input type="text" style="width: 51%"  id="adjustment_amount" onchange="setNewPayableAmount()"/>									
						 ...
						 <div id="othersAmount_div" style="display: none;padding-top: 20px;">
						 <table  id="othersTable" style="width: 100%">
						 	<tr valign="top">
								<th scope="row" width="23.5%"></th>
								<td width="75%">
									...
									<a class="addCF" href="javascript:void(0);" style='margin-left:15px;font-weight: bold;'>Add</a>
								</td>
							</tr>
						...
						<table style="width: 100%">
							...
							<tr>
								<th style="font-weight: bold;">Total Comment :</th>
								<td id="totalComment" style="padding-top: 4px;">
								</td>
							</tr>
						 ...
						 <div class="formSep" style="padding-top: 2px;padding-bottom: 2px;">
							...
							<div class="formSep sepH_b" style="padding-top: 3px;margin-bottom: 0px;padding-bottom: 2px;">									
								<button class="btn btn-beoro-3" type="button" id="btn_save" onclick="saveOthersAmount()">Save Others Amount</button>
							</div> 
						 </div>
...
$("#unpaid_bill_grid").jqGrid($.extend(true, {}, scrollPagerGridOptions, {
	url: jsEnum.GRID_RECORED_FETCHER+'?service=org.jgtdsl.models.BillingService&method=getBillList',
   	jsonReader: {
            repeatitems: false,
            id: "bill_id"
	},
   colNames: ['Customer Id','Bill Id','Bill Month', 'Bill Year','Bill Amount','Adjustment Amount', 'Payable Amount','Comments','↓'],
   colModel: [{
	                name: 'customer_id',
	                hidden:true
            	},
	...
	width: $("#right_div").width()+10,
   	...
   	sortname: 'bill_id',
    sortorder: "asc",
	caption: "Non-Approved Bill/Invoice List",
...
function fetchBillInfo(bill_id,customer_id){
	  ...
	  $.ajax({
		    url: 'getBillInfoForAdjustment.action',
		    type: 'POST',
...
function setBillInfo(bill){
	...
	$("#payable_amount").val(bill.payable_amount-bill.adjustment_amount);
	...
	$("#new_payable_amount").val(bill.payable_amount);
...
if($("#adjustment_sign").val()=="") return;
if($("#adjustment_sign").val()=="+")
 newPayableAmount=parseFloat($("#payable_amount").val(),10)+parseFloat($("#adjustment_amount").val(),10);
else if($("#adjustment_sign").val()=="-")
 newPayableAmount=parseFloat($("#payable_amount").val(),10)-parseFloat($("#adjustment_amount").val(),10);	
 $("#new_payable_amount").val(newPayableAmount);
}

function saveAdjustment(){
	if($("#adjustment_sign").val()=="" || $("#adjustment_amount").val()=="" || $("#adjustment_sign").val()=="") return;
	
	$.ajax({
		    url: 'saveAdjustmentInfo.action',
		    type: 'POST',
		    data: {bill_id:$("#bill_id").val(),customer_id:$("#customer_id").val(),adjustment_sign:$("#adjustment_sign").val(),adjustment_amount:$("#adjustment_amount").val(),remarks:$("#remarks").val(),metered_status:$("#isMetered_str").val()},
		    cache: false,
		    success: function (response) {
