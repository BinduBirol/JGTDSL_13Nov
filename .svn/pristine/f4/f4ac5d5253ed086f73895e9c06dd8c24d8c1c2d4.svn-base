...
public class Customer extends BaseAction implements SessionAware{
	...
	{
		ResponseDTO response;
		response=CustomerService.updateCustomer_note(customer_id, customer_note);
		setJsonResponse(response);		
		//CacheUtil.clear("CUSTOMER_INFO_"+customer.getCustomer_id());
		return null;
	}
	...
	public String noteView()	
	{
		...
		CustomerService cs= new CustomerService();
		this.customer_note=cs.getCustomerNote(customer_id);					
		return SUCCESS;
	}
	...
	{
		DepositService ds= new DepositService();
		this.depositList= ds.getCustomerDepositInfo(customer_id);
		return SUCCESS;
	}
	...
	public String getCollectionLedger(){
		
		LedgerService customerLedger=new LedgerService();
		ArrayList<CollectionLedgerDTO> customerList=customerLedger.getCollectionLedger(customer_id);		
		Gson gson = new Gson();
