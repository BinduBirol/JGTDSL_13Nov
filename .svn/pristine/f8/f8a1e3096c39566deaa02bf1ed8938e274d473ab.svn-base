...
$("#comm_customer_id").autocomplete($.extend(true, {}, acMCustomerOption,{
    	...
    	onSelect:function (){getCustomerInfo("comm",$('#comm_customer_id').val(),getSecurityBalance, reloadBillGrid());
		}
}));
...
function getSecurityBalance(){
		...
		$.ajax({
			    ...
			    success: function (response) {
			    	...
			    	$("#totalsecurityAmount").val(response.security);
			    	$("#totalAdjustableAmount").val(response.security);
...
duesGrid.jqGrid({
  ...
  colNames: ['Bill Id','Month','Year','Bill Month Year','Bill Amount','Collected Amount','Surcharge', 'Due Amount','Adjust Amount','Surcharge','Bill Type'],
  colModel: [{    name: 'bill_id',
          	...
          	
          	{
            	...
            	
            	{
	                ...
	                align:'center',
	                formatter: function (cellValue, option,rowObject) {
        				return '<input  type="text" maxlength="8" id="surcharge_'+rowObject.bill_id+'" style="width:70px;height:10px;text-align:right;" onblur="calculateTotalAjustment('+rowObject.bill_id+',\'blurEvent\')"/>'+
        					   '<input type="hidden" id="bill_id_'+rowObject.bill_id+'" value="'+rowObject.bill_id+'" />'+
        					   '<input type="hidden" id="bill_type_'+rowObject.bill_id+'" value="'+rowObject.bill_type+'" />';
    				} 
            	},
...
function reloadBillGrid() {
	...
	var ruleArray = [ [ "customer_id" ], [ "eq" ],
			[ $('#comm_customer_id').val() ] ];
	var $grid = $('#bill_grid');
	var caption_extra = "";

	var postdata = getPostFilter("dues_bill_grid", ruleArray);
	$("#dues_bill_grid").jqGrid('setGridParam', {
		search : true,
		postData : postdata,
		page : 1,
		datatype : 'json'
	...
	$grid.jqGrid('setCaption', 'Due bill list for - '
					+ $('#customer_id').val());
}
...
function calculateTotalAjustment(rowId,type) {	
	...
	billIds="";
		...
		for(a=0;a<rows.length;a++)
		 {	
		    ...
		    { 
				...
				if(type=="checkEvent"){
					$("#adjust_amount_"+rowId).val(parseFloat(rowData["payable_amount"])-parseFloat(rowData["collected_amount"]));
					$("#surcharge_"+rowId).val(parseFloat(rowData["surcharge_actual"]));
				}
				...
				totalSurchargeAmount = parseFloat(totalSurchargeAmount)+  parseFloat($("#surcharge_"+rowData["bill_id"]).val());	
		...
		duesGrid.jqGrid("footerData", "set", {bill_year: "Total:", payable_amount: totalBill,collected_amount: totalCollected,remaining_amount: totalDues,adjust_amount:"", surcharge_amount:""});
		$("#adjust_amount_undefined").val(totalAdjustAmount);
		$("#adjust_amount_undefined").prop('disabled', true); 
		
		$("#surcharge_undefined").val(totalSurchargeAmount);
		$("#surcharge_undefined").prop('disabled', true); 
...
function validateSecurityAdjustment(){	
	...
	if(document.getElementById("adjustmentMode_refund").checked){
		...
		*/
		var rowz = jQuery("#dues_bill_grid").getDataIDs();
		if(rowz!=""){
			alert("Unable to refund. Due bill found!!");
			return;
		}
	}
...
function saveSecurityAdjustment(){
	...
	for(var a=0;a<rows.length;a++)
	 {	
	    ...
	    { 
	    	adjustmentBillStr=adjustmentBillStr+"1#"+row.bill_id+"#"+$("#adjust_amount_"+row.bill_id).val()+"#"+$("#surcharge_"+row.bill_id).val()+"@";
	    }
	}
	...
	disableButton("btn_save");
	  ...
	  $.ajax({
		    ...
		    type: 'POST',
		    data: {customerId:$("#comm_customer_id").val(),isMeter:$("#comm_isMetered_name").val(),securityAmount:$("#totalsecurityAmount").val(),
