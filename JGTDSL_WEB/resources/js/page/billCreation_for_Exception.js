...
Calendar.setup({
	...
	Calendar.setup({
    inputField : "bill_due_date_w_sur",
    trigger    : "bill_due_date_w_sur",
	eventName : "focus",
    onSelect   : function() { this.hide()},
    showTime   : 12,
    dateFormat : "%d-%m-%Y",
...
function validateAndProcessBilling()
{
 ...
 if(isValid==true)	 {
	...
	var formData = new FormData(form);
	  $.ajax({
	    url: 'processBillException.action',
	    type: 'POST',
	    data: formData,
	    async: false,
	    cache: false,
...
$.ajax({
    url: 'getExceptionReading.action',
    type: 'POST',
    ...
    cache: false,
    success: function (response) {     	
    	$("#consumption").val(response.actual_consumption);
    	$("#p_reading_date").val(response.curr_reading_date);
    	$("#previous_reading").val(response.prev_reading);
    	$("#previous_reading_date").val(response.prev_reading_date);
    	$("#p_reading").val(response.curr_reading);
    	$("#rate").val(response.rate);
    	$("#m_rent").val(response.meter_rent);
    	$("#hnv").val(response.hhv_nhv);
    }
