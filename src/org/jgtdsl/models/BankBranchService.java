...
public class BankBranchService {
	...
	public static ArrayList<BankDTO> getBankList(int index, int offset,String whereClause,String sortFieldName,String sortOrder,int total)
	{
		...
		if(whereClause.contains("AREA_ID") & !whereClause.contains(".AREA_ID"))
			whereClause=whereClause.replace("AREA_ID", "bank.AREA_ID");
		...
		//end of additional code
				  ...
				  sql = " Select distinct bank.bank_id,bank.AREA_ID,bank_name,bank.address,bank.phone," +
		   ...
		   ResultSet r = null;
			try
			{
				...
				while (r.next())
				{
					...
					bank.setArea_id(r.getString("AREA_ID"));
	...
	////for collection 
	public static ArrayList<BankDTO> getBankListforCol(int index, int offset,String whereClause,String sortFieldName,String sortOrder,int total)
	{
		BankDTO bank=null;
		ArrayList<BankDTO> bankList=new ArrayList<BankDTO>();
		Connection conn = ConnectionManager.getConnection();
		...
		if(total==999999)
			     sql="  SELECT DISTINCT bank.bank_id, " +
			    		 ...
			    		 "                  bank.status " +
			    		 "    FROM MST_BANK_INFO bank, MST_BRANCH_INFO branch " +
			    		 "   WHERE bank.bank_id = branch.bank_id and bank.status<>0 "+
			    		 (whereClause.equalsIgnoreCase("")?"":(" And ( "+whereClause+" )"))+
			    		 " "+orderByQuery ;
		//end of additional code
		else if(total==0)
				  sql = " Select distinct bank.bank_id,bank_name,bank.address,bank.phone," +
				  		"bank.fax,bank.email,bank.url,bank.description,bank.status " +
				  		"from MST_BANK_INFO bank,MST_BRANCH_INFO branch " +
				  		"Where bank.bank_id=branch.bank_id  and bank.status<>0 "+(whereClause.equalsIgnoreCase("")?"":(" And ( "+whereClause+" )"))+" "+orderByQuery;
		else
				  sql=" Select * from ( " +
				  	  " Select rownum serial,tmp1.* from " +
				  	  " ( Select distinct bank.bank_id,bank_name,bank.address,bank.phone,bank.fax,bank.email,bank.url,bank.description,bank.status from MST_BANK_INFO bank,MST_BRANCH_INFO branch Where bank.bank_id=branch.bank_id and bank.status<>0 "+(whereClause.equalsIgnoreCase("")?"":(" And ( "+whereClause+")"))+" "+orderByQuery+			  	  
				  	  "    )tmp1 " +
				  	  "    )tmp2   " +
				  	  "  Where serial Between ? and ? ";
	...
	public String addBank(String data)
	{
		...
		PreparedStatement stmt = null;
			try
			{
				...
				stmt.setString(10, bankDTO.getArea_id());
	...
	public String updateBank(String data)
	{
		...
		BankDTO bankDTO = gson.fromJson(data, BankDTO.class);  	
		Connection conn = ConnectionManager.getConnection();
		String sql=" Update MST_BANK_INFO Set BANK_NAME=?,ADDRESS=?,PHONE=?,FAX=?,EMAIL=?,URL=?,DESCRIPTION=?,STATUS=? Where BANK_ID=? and area_id=? ";
		int response=0;
		PreparedStatement stmt = null;
			try
			{
	...
	public ArrayList<BranchDTO> getBranchList(int index, int offset,String whereClause,String sortFieldName,String sortOrder,int total)
	{
		...
		if(total==0)
				  sql = " Select * from MST_BRANCH_INFO  "+(whereClause.equalsIgnoreCase("")?"":("Where ( "+whereClause+" and status=1)"))+" ";
		else
				  sql=" Select * from ( " +
				  	  " Select rownum serial,tmp1.* from " +
...
{  	
	Connection conn = ConnectionManager.getConnection();
	String sql=" select lpad(max(to_number(BANK_ID))+1,2,'0') ID from MST_BANK_INFO where bank_id not like '%S%' ";
	String response="";
	PreparedStatement stmt = null;
	ResultSet r = null;
...
public static ArrayList<BankDTO> mpgBankList(){
	
	BankDTO bankDTO=null;			
	ArrayList<BankDTO> mpg_banks=new ArrayList<BankDTO>();
	Connection conn = ConnectionManager.getConnection();
	String sql= " select FULLNAME from MPG_USERINFO ";		
	   ...
	   ResultSet r = null;
		try
		{
			...
			{
				bankDTO=new BankDTO();
				bankDTO.setBank_name(r.getString("FULLNAME"));	
				mpg_banks.add(bankDTO);
