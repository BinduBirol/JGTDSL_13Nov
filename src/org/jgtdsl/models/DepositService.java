...
public class DepositService {
	...
	/////deposit purpose	
	public static ArrayList<DepositTypeDTO> getDepositPurposeList() {
		DepositTypeDTO depositDTO = null;
		ArrayList<DepositTypeDTO> depositTypeList = new ArrayList<DepositTypeDTO>();
		Connection conn = ConnectionManager.getConnection();
		String sql = " select * from MST_DEPOSIT_TYPES where status=1 order by TYPE_NAME_ENG ";
...
//								|| Integer.valueOf(deposit.getStr_deposit_type()) == DepositType.FDR.getId()) ? account_id_bg : deposit.getAccount_no());
			...
			if (Integer.valueOf(deposit.getStr_deposit_type()) == DepositType.CASH_BANK.getId())
			{
				...
				if(deposit.getStr_deposit_purpose().equals("1"))
				{
...
//				+" AND CSL.DEPOSIT_ID = MD.DEPOSIT_ID )";
		...
		String mstDepositSql = "Update MST_DEPOSIT Set  VALID_FROM=TO_DATE(?,'DD-MM-YYYY'),VALID_TO=TO_DATE(?,'DD-MM-YYYY'),BANK_ID=?,BRANCH_ID=?,ACCOUNT_NO=?,TOTAL_DEPOSIT=?,DEPOSIT_DATE=TO_DATE(?,'DD-MM-YYYY') Where Deposit_ID=?";
...
//			security_history_stmt.execute();
			...
			mst_stmt.setString(7, deposit.getDeposit_date());			
			mst_stmt.setString(8, deposit.getDeposit_id());
			mst_stmt.execute();
...
//			deposit_ledger_stmt.execute();
	...
	public ArrayList<DepositDTO> getDepositList(String customer_id) {
		...
		String sql = "SELECT md.*, "
				+ "         DT.TYPE_NAME_ENG, "
				+ "         TO_CHAR (DEPOSIT_DATE, 'DD-MM-YYYY') DEPOSIT_DATE_ "
				+ "    FROM MST_DEPOSIT md, MST_DEPOSIT_TYPES dt "
				+ "   WHERE Customer_Id = ? AND DT.TYPE_ID = MD.DEPOSIT_PURPOSE "
		...
		try {
			...
			while (r.next()) {
				...
				deposit.setStr_deposit_purpose(r.getString("TYPE_NAME_ENG"));
				deposit.setDeposit_date(r.getString("DEPOSIT_DATE_"));
				/* muted by bindu with anger :{
	...
	public DepositDTO getDepositDetail(String deposit_id) {
		...
		String sql = "SELECT tmp3.*, account_name "
				...
				+ "                                        'dd-MM-YYYY' "
				+ "                                       ) deposit_date, TYPE_NAME_ENG, "
				+ "                               total_deposit "
				+ "                          FROM mst_deposit, MST_DEPOSIT_TYPES "
				+ "                         WHERE deposit_id = ? AND TYPE_ID = DEPOSIT_PURPOSE ) tmp1 "
				+ "                       LEFT OUTER JOIN "
	...
	public ArrayList<DepositDTO> getBGExpireWithinOneYear() {
		...
		try{
			String sql = "SELECT md.customer_id,MV.FULL_NAME, "
					+ "       md.deposit_id,"
					+ "       getbankbranch (branch_id) BankandBranchName, "
					+ "       account_no, "
					+ "       MV.CATEGORY_ID, "
					+ "       MV.CATEGORY_NAME, "							
					+ "       total_deposit,TO_CHAR (VALID_FROM, 'dd-MM-YYYY') valid_from, "
					+ "       TO_CHAR (valid_to, 'dd-MM-YYYY') valid_to, "
					+ "       valid_to - TRUNC (SYSDATE) expire_in "
					+ "  FROM mst_deposit md, mview_customer_info mv "
					+ " WHERE MD.CUSTOMER_ID = MV.CUSTOMER_ID and MV.AREA_ID = '"+area_id+"'  AND Deposit_Type = 1 AND ( (valid_to - TRUNC (SYSDATE)) <365 )  ";
		...
		//CustomerDTO cusDTO = new CustomerDTO();
			...
			while (r.next()) {
				...
				deposit.setCustomer_id(r.getString("CUSTOMER_ID"));
				deposit.setCustomer_name(r.getString("FULL_NAME"));				
				deposit.setBank_name(r.getString("BankandBranchName"));				
				deposit.setTotal_deposit(r.getString("TOTAL_DEPOSIT"));
				deposit.setValid_from(r.getString("valid_from"));
				deposit.setValid_to(r.getString("VALID_TO"));
				deposit.setExpire_in(r.getString("EXPIRE_IN"));
	...
	public ArrayList<DepositDTO> getBGExpireWithinOneYear(String customer_id_for_bg) {
		DepositDTO deposit = null;
		ArrayList<DepositDTO> expireList = new ArrayList<DepositDTO>();
		Connection conn = ConnectionManager.getConnection();
		...
		try{
			String sql = "SELECT deposit.deposit_id, " +
					...
					" AND CUS.CUSTOMER_ID= '"+customer_id_for_bg+"' "+
					//"         AND ( (valid_to - TRUNC (SYSDATE)) <= 365) " + by bindu
	...
	public ArrayList<DepositDTO> getBGExpireBydays(String days) {
		DepositDTO deposit = null;
		ArrayList<DepositDTO> expireList = new ArrayList<DepositDTO>();
		Connection conn = ConnectionManager.getConnection();
		...
		try{			
			...
			String sql=  	"SELECT md.customer_id,MV.FULL_NAME, "
							...
							+ "       valid_to - TRUNC (SYSDATE) expire_in "
							+ "  FROM mst_deposit md, mview_customer_info mv "
							+ " WHERE MD.CUSTOMER_ID = MV.CUSTOMER_ID and MV.AREA_ID = '"+area_id+"'  AND Deposit_Type = 1   ";
...
//				}
	...
	public ArrayList<DepositDTO> getCustomerDepositInfo(
			String customer_id) {
	
		ArrayList<DepositDTO> DepositTtotalList = new ArrayList<DepositDTO>();
		Connection conn = ConnectionManager.getConnection();
		
		String sql = " SELECT SUM (TOTAL_DEPOSIT) total_deposit, " +
		...
		PreparedStatement stmt = null;
		ResultSet r = null;
		DepositDTO depositTotal = null;
		try {
			stmt = conn.prepareStatement(sql);
			stmt.setString(1, customer_id);
			r = stmt.executeQuery();
			while (r.next()) {
				depositTotal = new DepositDTO();
				depositTotal.setDeposit_id(r.getString("deposit_type"));
				depositTotal.setTotal_deposit(r.getString("total_deposit"));
				
				DepositTtotalList.add(depositTotal);
