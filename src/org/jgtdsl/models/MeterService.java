...
public class MeterService {
	...
	public ArrayList<CustomerMeterDTO> getCustomerMeterList(String customer_id,
		...
		if (meter_id == null || meter_id.equalsIgnoreCase(Utils.EMPTY_STRING))
			sql = "Select tmp1.customer_id,getMinMaxLoad(meter_id) as MinMaxLoad, meter_id,meter_sl_no,meter_mfg,mfg_name,meter_year,measurement_type, tmp1.unit, "
			...
			sql = " SELECT * "
					+ "        FROM (Select tmp1.customer_id, getMinMaxLoad(meter_id) as MinMaxLoad,meter_id,meter_sl_no,meter_mfg,mfg_name,meter_year,measurement_type, tmp1.unit, "
					+ "        evc_sl_no,evc_model,evc_year,meter_type,type_name meter_type_name,g_rating,rating_name g_rating_name,conn_size,max_reading, "
	...
	public ArrayList<CustomerApplianceDTO> getCustomerApplianceList(
		...
		sql = "SELECT BQC.PID, "
				...
				+ "        nvl( BQC.TOTAL_TDISCONNECTED_QNT,0)-((BQC.NEW_APPLIANCE_QNT_BILLCAL-BQC.NEW_APPLIANCE_QNT)*2) TOTAL_TDISCONNECTED_QNT "
				+ "  FROM BURNER_QNT_CHANGE BQC,APPLIANCE_INFO AI "
				+ "  WHERE   BQC.APPLIANCE_TYPE_CODE=AI.APPLIANCE_ID AND AREA_ID= substr(customer_id,1,2) AND PID IN "
				+ "          (SELECT MAX (PID) "
