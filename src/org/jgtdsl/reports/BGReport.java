...
public class BGReport extends BaseAction {
	...
	private void generatePdf_for_BGExpireListWithIn365days(Document document) throws DocumentException {
		...
		if(days.isEmpty()){
			...
			expireList = depositeService.getBGExpireBydays(days);
		}
		String name="";
		if(days.isEmpty()){
		 name ="365 days";
		...
		pcell = new PdfPCell(new Paragraph("Bank Guarantee will expire within  "+name+"  ", ReportUtil.f8B));
		...
		pcell = new PdfPCell(new Paragraph("Date : "+formattedDate));
		pcell.setMinimumHeight(18f);
		...
		pdfPTable.setWidths(new float[] { 5, 10, 10, 15, 20, 10, 10, 10,10 });
		...
		pcell = new PdfPCell(new Paragraph("Valid From", ReportUtil.f9B));
		pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
		pcell.setColspan(1);
		pdfPTable.addCell(pcell);
		...
		for (int j = 0; j < expireListSize; j++) {
			...
			pcell = new PdfPCell(new Paragraph(expireList.get(j).getValid_from(),
					ReportUtil.f9));
			pcell.setHorizontalAlignment(Element.ALIGN_LEFT);
			pcell.setColspan(1);
			pdfPTable.addCell(pcell);
			...
			//System.out.println(expireList.get(j).getTotal_deposit());
			
			total_deposit+= Double.parseDouble(expireList.get(j).getTotal_deposit());
		...
		pcell = new PdfPCell(new Paragraph(String.valueOf(taka_format.format(total_deposit)), ReportUtil.f9B));
		pcell.setHorizontalAlignment(Element.ALIGN_LEFT);
		pcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
		pcell.setColspan(4);
		pcell.setPadding(5);
