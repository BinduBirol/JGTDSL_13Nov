...
// for logo			
			...
			pcell=new PdfPCell(new Paragraph("Codeless Collection Report For: "+Month.values()[Integer.valueOf(bill_month)-1]+", "+bill_year, font3));
			pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
			pcell.setBorder(Rectangle.NO_BORDER);
			...
			for(CodelessDTO x: codelessCollectionInfo){
				...
				String from_to_month_year="";
				if(x.getMonth_year_from().equalsIgnoreCase(x.getMonth_year_to())){
					from_to_month_year= x.getMonth_year_to();
				}else{
					from_to_month_year=x.getMonth_year_from()+" - "+x.getMonth_year_to();
				}
				
				pcell= new PdfPCell(new Paragraph(from_to_month_year, font2));
				pcell.setHorizontalAlignment(Element.ALIGN_LEFT);
				pcell.setVerticalAlignment(Element.ALIGN_MIDDLE);			
				pcell.setPadding(5);			
				dataTable.addCell(pcell);
	...
	public ArrayList<CodelessDTO> getCodelessCollectionDetais() {
		...
		String sql=  
						...
						"   WHERE     TO_CHAR (COLLECTION_DATE, 'mm') =  " +this.getBill_month()+
						"         AND TO_CHAR (COLLECTION_DATE, 'YYYY') = " +this.getBill_year()+
						"         AND area_id = " +this.getArea()+
		...
		try {
			...
			while (r.next()) {
				...
				String from_month_year= String.valueOf(Month.values()[Integer.valueOf(r.getString("MONTH_FROM"))-1])+","+r.getString("YEAR_FROM"); 				
				String to_month_year= String.valueOf(Month.values()[Integer.valueOf(r.getString("MONTH_TO"))-1])+","+r.getString("YEAR_TO");
				
				codelessDTO.setMonth_year_from(from_month_year);
				codelessDTO.setMonth_year_to(to_month_year);
